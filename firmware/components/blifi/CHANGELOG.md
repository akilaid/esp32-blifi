...
### Fixed
- Registry publish reliability: reverted the `description` field that 0.1.2 added
  to each example's `main/idf_component.yml`. That non-standard field (no
  well-maintained component ships one) broke server-side example processing, so
  0.1.2 uploaded with a processing error and the example blurbs never applied.
- Example manifests now pin the back-dependency as `akilaid/blifi: "^0.1"`
  instead of `"*"`, matching espressif/led_strip and esp-iot-solution/button, so
  the registry resolves it cleanly while packing the examples.
